...
void beams(Joint* pJoint,int iro) {
	CameraSpacePoint right = pJoint[JointType_HandRight].Position;
	CameraSpacePoint hright = pJoint[JointType_HandTipRight].Position;
	CameraSpacePoint elbow = pJoint[JointType_ElbowRight].Position;
	CameraSpacePoint shoulder = pJoint[JointType_ShoulderRight].Position;
	...
	if (cosine(dir0, dir1) > 0.3 ) {
		...
		for (int i = 0; i < 100; i++) {
			...
			glVertex3f(right.X + dir[0] * len + vec[0] * wide,
				right.Y + dir[1] * len + vec[1] * wide,
				right.Z + dir[2] * len + vec[2] * wide);
		}
		glEnd();
	}
}
...
void display() {
	...
	for (int i = 0; i < BODY_COUNT; i++) {
		...
		if (pBody[i] != nullptr) {
			HandState handstate;
			pBody[i]->get_HandRightState(&handstate);
			int iro;
			if (handstate==2) {
				iro = 0;
				pBody[i]->GetJoints(JointType_Count, pJoint);
